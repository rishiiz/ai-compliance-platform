...
def get_current_user(
    ...
    """Dependency: auth disabled — always returns the default admin user."""
    # Authentication is disabled. Try token-based auth first, fall back to default admin.
    if authorization and authorization.startswith("Bearer "):
        ...
        if user_id:
            ...
            if user:
                return user
    # No token or invalid token — return default admin user
    user = User.objects(email="admin@company.com").first()
    if not user:
        raise HTTPException(status_code=500, detail="Default admin user not found. Ensure the backend seeded correctly.")
    return user
